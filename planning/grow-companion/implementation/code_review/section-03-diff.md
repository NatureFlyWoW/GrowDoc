...
index 0000000..42a8d4e
--- /dev/null
+++ b/css/onboarding.css
@@ -0,0 +1,262 @@
+/* GrowDoc Companion — Onboarding Wizard Styles */
+
+/* Landing page */
+.landing {
+  display: flex;
...
+}
+
+/* Selection cards (stage, medium, lighting, experience) */
+.selection-cards {
+  display: grid;
...
+
+.star-btn.filled {
+  color: var(--star-color, var(--accent-green));
+}
+
...
+
+  if (_wizardState.step > 1 && _wizardState.step < TOTAL_STEPS) {
...
+  completeBtn.addEventListener('click', () => _completeOnboarding(container));
...
+      strainId: null,
+      strainCustom: w.strainName ? { name: escapeHtml(w.strainName) } : null,
+      potSize: w.potSize,
+      stage: w.stage,
